...
public class SettingsV2
{
    ...
    public SettingsV2(Uri url, string apiKey, string issuer, string audience, string jwksUrl, bool enableAsymmetricKeys = true)
    {
        Url = url;
        ApiKey = apiKey;
        Issuer = issuer;
    ...
    public static SettingsV2 FromUrl(Uri url, string apiKey, string audience = "authenticated", bool enableAsymmetricKeys = true)
    {
        var baseUrl = url.ToString().TrimEnd('/');
        var issuer = $"{baseUrl}/auth/v1";
        var jwksUrl = $"{baseUrl}/auth/v1/.well-known/jwks.json";

        return new SettingsV2(url, apiKey, issuer, audience, jwksUrl, enableAsymmetricKeys);
    }
    ...
    /// <param name="enableAsymmetricKeys">Whether to enable asymmetric keys (default: true)</param>
    /// <returns>A new Settings instance</returns>
    public static SettingsV2 FromUrl(string url, string apiKey, string audience = "authenticated", bool enableAsymmetricKeys = true)
    {
